...
class Layout {
    ...
    private AtasSsg $atasSsg;

    public function __construct(AtasSsg $atasSsg)
    {

        $this->atasSsg = $atasSsg;
    }
    ...
    function GetProcessedPageMeta(): ProcessedPageMeta
    {
        ...
        if (!preg_match('/^(https?:\/\/|\/)/', $page_meta->og_image)) {
            ...
            $processed->og_image = $this->atasSsg->getCurrentHostnameWithProtocol() . "/" . $page_meta->og_image;
        }
